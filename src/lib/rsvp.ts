 ...
 * window length — if it ever does, read it from the event payload and fall back to this.
 */
 ...
 * 24-hour format" — neither carries a zone, so they mean local time to whoever typed
 ...
 * else is decided by status alone — notably NOT by the clock, so an event still PUBLISHED
 ...
 * the same moment — see docs/RSVP_LATE_REGISTRATION.md §4 for how that was confirmed.
 */
